...
class RemoteExplorer implements vscode.TreeDataProvider<RemoteNode> {
  ...
  async getChildren(node?: RemoteNode): Promise<RemoteNode[]> {
    ...
    } catch (error) { vscode.window.showErrorMessage(`Remote Deploy: ${messageOf(error)}`); return []; }
  }
}
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('remoteDeploy.upload', async (uri?: vscode.Uri, profileOverride?: DeploymentProfile) => withLocalFile(uri, async local => { const profile = profileOverride ?? await selectProfileForLocalAction('upload to'); if (!profile) { return; } const remote = remotePathFor(profile, local.fsPath); await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Uploading ${path.basename(local.fsPath)} to ${profile.name}` }, () => sftp.upload(profile, local.fsPath, remote)); explorer.refresh(); vscode.window.showInformationMessage(`Uploaded to ${profile.name}: ${remote}`); })),
...
function openSyncLocalPreview(context: vscode.ExtensionContext, profile: DeploymentProfile, files: SyncLocalPreviewFile[]): void {
  ...
  panel.webview.onDidReceiveMessage(async (message: { type: string; index?: number }) => {
    ...
    if (message.type === 'sync') {
      ...
      } catch (error) { panel.webview.postMessage({ type: 'error', index: message.index ?? 0, text: `Sync failed: ${messageOf(error)}` }); }
      return;
    }
...
function openDeployPreview(context: vscode.ExtensionContext, sftp: SftpService, explorer: RemoteExplorer, profile: DeploymentProfile, files: DeployPreviewFile[]): void {
  ...
  panel.webview.onDidReceiveMessage(async (message: { type: string; index?: number }) => {
    ...
    if (message.type === 'deploy') {
      ...
      } catch (error) { panel.webview.postMessage({ type: 'error', index: message.index ?? 0, text: `Deployment failed: ${messageOf(error)}` }); }
      return;
    }
    if (message.type === 'overwriteLocal') {
      ...
      } catch (error) { panel.webview.postMessage({ type: 'error', index: message.index ?? 0, text: `Overwrite local failed: ${messageOf(error)}` }); }
      return;
    }
...
function syncLocalPreviewHtml(profile: DeploymentProfile, files: SyncLocalPreviewFile[]): string {
  const file = files[0];
  if (!file) { return ''; }
...
function deployPreviewHtml(profile: DeploymentProfile, files: DeployPreviewFile[]): string {
...
function openProfilesPanel(context: vscode.ExtensionContext, sftp: SftpService, explorer: RemoteExplorer): void {
  ...
  panel.webview.onDidReceiveMessage(async (message: { type: string; profiles?: DeploymentProfile[]; defaultProfileId?: string; profile?: DeploymentProfile }) => {
...
function profilePanelHtml(settings: Settings, workspacePath: string): string {

...
function activate(context) {
    ...
    let lastSelectedTree;
    const selectedRemoteNodes = () => {
        if (lastSelectedTree?.selection.length) {
            return lastSelectedTree.selection;
        }
        if (primaryTree.visible && primaryTree.selection.length) {
            return primaryTree.selection;
        }
        if (fallbackTree.visible && fallbackTree.selection.length) {
            return fallbackTree.selection;
        }
        return primaryTree.selection.length ? primaryTree.selection : fallbackTree.selection;
    };
    const selectedRemoteNode = () => selectedRemoteNodes()[0];
...
function openSyncLocalPreview(context, profile, files) {
    ...
    panelDisposables.push(panel.webview.onDidReceiveMessage(async (message) => {
        ...
        if (message.type !== 'sync' && message.type !== 'syncAll') {
            return;
        }
        if (operationBusy) {
            return;
...
function openDeployPreview(context, sftp, explorer, profile, files) {
    ...
    panelDisposables.push(panel.webview.onDidReceiveMessage(async (message) => {
        ...
        if (message.type !== 'deploy' && message.type !== 'overwriteLocal' && message.type !== 'deployAll') {
            return;
        }
        if (operationBusy) {
            return;
...
function previewSyncState(file) {
...
function previewDeployState(file) {
...
function syncLocalPreviewHtml(cspSource, profile, files) {
...
function deployPreviewHtml(cspSource, profile, files) {
    const nonce = webviewNonce();
...
function previewHtml(cspSource, nonce, title, description, data, mode) {
...
function openProfilesPanel(context, sftp, explorer) {
    ...
    panelDisposables.push(panel.webview.onDidReceiveMessage(async (message) => {
        ...
        if (message.type === 'test' && message.profile) {
            try {
                await sftp.dispose();
                await sftp.list({ ...normalizeProfile(message.profile), password: message.profile.password }, normalizeRemotePath(message.profile.remoteRoot || '/'));
...
function profilePanelHtml(cspSource, settings, workspacePath) {
    const nonce = webviewNonce();
...
function selectServerInEditor(_context, settings, title, description) {
    ...
    return new Promise(resolve => {
        ...
        const panelDisposables = [];
        let settled = false;
        const finish = (profile) => { if (settled) {
            return;
        } settled = true; resolve(profile); panel.dispose(); };
        panelDisposables.push(panel.onDidDispose(() => { panelDisposables.splice(0).forEach(disposable => disposable.dispose()); if (!settled) {
            settled = true;
            resolve(undefined);
        } }));
        panelDisposables.push(panel.webview.onDidReceiveMessage((message) => {
            ...
            if (message.type === 'configure') {
                finish();
                void vscode.commands.executeCommand('remoteDeploy.configure');
                return;
            }
            if (message.type === 'select' && message.id) {
                finish(settings.profiles.find(profile => profile.id === message.id));
            }
        }));
        panel.webview.html = serverSelectorHtml(panel.webview.cspSource, title, description, projection);
    });
}
async function selectProfileForLocalAction(context, action) { const settings = getSettings(); if (!settings.profiles.length) {
    vscode.window.showWarningMessage('Create an SFTP profile first.');
    return undefined;
} return selectServerInEditor(context, settings, 'Select Server', `Select the server to ${action}.`); }
function serverSelectorHtml(cspSource, title, description, profiles) {
    const nonce = webviewNonce();
